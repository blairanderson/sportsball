...
fs.writeFileSync('./mlb/sked/total.csv', header)
async.each(files, function(file, callback) {
    ...
    request(fileUrl, function (error, response, html) {
      ...
      if (error) {
        ...
        var stuff = html.replace(/['"]+/g, '');
        fs.appendFileSync('./mlb/sked/total.csv', stuff)
        fs.writeFile(fileOutput, (header + stuff), callback)
      }
    });
}, function(err) {
